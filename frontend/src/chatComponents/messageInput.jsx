...
import { Send, Paperclip, Smile, X, FileText, Film } from 'lucide-react';
...
const MessageInput = ({ chatId, overrideOnSend, targetUserId }) => {
  ...
  // ── Helper: always use uppercase chatID to match server ────────
  const emitStopTyping = () => {
    if (!targetUserId || !chatId) return;
    socket.emit('stop-typing', {
      targetUserId,
      chatID: chatId, // ✅ uppercase D — matches server destructuring
    });
  ...
  useEffect(() => {
    return () => {
      ...
      // Clear ghost typing indicator when switching chats
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // ✅ FIX: was `chatId` (lowercase) before — server receives `chatID`
      // so it got undefined and never cleared the typing indicator
      emitStopTyping();
      clearTimeout(typingTimeoutRef.current);
      setMessage('');
      setSending(false);
      ...
      <div className="px-5 py-4">
        <div className={`flex items-center gap-2 bg-[#f7f8fc] border rounded-2xl
          ...
          <button type="button"
                  ...
                  className={`transition-colors shrink-0 ${file ? 'text-[#00b87a]' : 'text-gray-400 hover:text-gray-600'}`}
                  title="Attach file">
            <Paperclip size={17} />
          </button>
          ...
          <button type="button"
                  ...
                  className={`transition-colors shrink-0 ${showEmoji ? 'text-[#00b87a]' : 'text-gray-400 hover:text-gray-600'}`}>
            <Smile size={17} />
          </button>
